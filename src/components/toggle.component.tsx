...
const Toggle = ({ movie }: { movie: Movie }) => {
  ...
  const { watchlist, favorites, toggleWatchlist, toggleFavorite } = useMovieStore();
  ...
  return (
    <>
      <span
        ...
        onClick={() => (isAuthenticated ? toggleWatchlist(movie) : setAuthModal(true))}
        className={styles.icon_watchlist}
      >
        {isWatchlist && isAuthenticated ? <FillWatchlist /> : <OutlineWatchlist />}
      </span>
      <span
        style={{ display: isFavoriteVisible ? 'block' : 'none' }}
        onClick={() => (isAuthenticated ? toggleFavorite(movie) : setAuthModal(true))}
        className={styles.icon_favorite}
      >
        {isFavorite && isAuthenticated ? <FilledFavorite /> : <OutlineFavorite />}
      </span>
    </>
  );
};
